...
private:
  ...
  void update(const aiwc::frame& f)
  {
    ...
    for(int i = 0; i <= 4; i++){
      ...
      double target[2];
      bool ball_to_penalty_area = false;
      ...
      if(i <= 1 || i == 4 
        ...
        if(i == 4){
          //ball_to_penalty_area = dist(target[0], target[1], -info.field[0] / 2.0, 0.0) <= dist(-info.field[0] / 2.0, 0.0, -info.field[0] / 2.0 + PENALTY_AREA_LENGTH, PENALTY_AREA_WIDTH / 2.0);
          if(!ball_to_penalty_area){
          ...
          //else std::cout << "Ball about to enter penalty_area: " << f.time << std::endl;
        }
      }
      else{
      ...
      if((new_dist <= last_dist[i] 
        || std::fabs(robot_spin_angle) <= (ANGLE_DIFF_MAX_TO_MOVE * ((i == 4 && ball_to_penalty_area) ? GK_ANGLE_AMPLIFIER : 1.0))) 
        && phase[i] >= 2) phase[i] = 2;
      //else if(std::fabs(robot_spin_angle) >= ANGLE_DIFF_MIN_TO_STOP && phase[i] > 1) phase[i] = 0;
      else if(std::fabs(robot_spin_angle) > (ANGLE_DIFF_MAX_TO_MOVE * ((i == 4 && ball_to_penalty_area) ? GK_ANGLE_AMPLIFIER : 1.0)) 
              && phase[i] == 1) phase[i] = 0;
      ...
      if(phase[i] == 0){
        ...
        wheels[2*i] = (move_forward ? 1.0 : -1.0) * info.max_linear_velocity * ((i == 4 && !ball_to_penalty_area) ? 2.0/3.0 : 1.0);
        wheels[2*i+1] = (move_forward ? 1.0 : -1.0) * info.max_linear_velocity * ((i == 4 && !ball_to_penalty_area) ? 2.0/3.0 : 1.0);
      }
...
private: // member variable
  ...
  const double GK_ANGLE_AMPLIFIER = 2.0;
